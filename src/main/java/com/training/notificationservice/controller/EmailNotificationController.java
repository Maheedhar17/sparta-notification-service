...
import com.training.notificationservice.dto.request.EmailNotificationRequestDto;
...
/**
 * Email-specific creation endpoint. Thin by design: validates the
 * email-shaped request, maps it onto the shared {@link NotificationRequestDto},
 * and delegates everything else (persistence, dispatch, status transitions) to
...
@Tag(name = "Email Notifications", description = "Email-specific notification creation")
public class EmailNotificationController {
    ...
    @PostMapping
    @Operation(summary = "Create and dispatch an email notification")
    public ResponseEntity<NotificationResponseDto> createEmailNotification(
            @Valid @RequestBody EmailNotificationRequestDto request) {
        log.info("POST /api/v1/notifications/email");
        NotificationResponseDto created = notificationService.createNotification(toGenericRequest(request));
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    private NotificationRequestDto toGenericRequest(EmailNotificationRequestDto request) {
        ...
        generic.setSubject(request.getSubject());
